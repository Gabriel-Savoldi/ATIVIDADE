...
export default function FormAlterarProduto(props) {
    const [produto, setProduto] = useState({
        codigo:props.produto.codigo,
        descricao:props.produto.descricao,
        precoCusto:props.produto.precoCusto,
        precoVenda:props.produto.precoVenda,
        qtdEstoque:props.produto.qtdEstoque,
        urlImagem:props.produto.urlImagem,
        dataValidade:props.produto.dataValidade

    });
    const [formValidado, setFormValidado] = useState(false);
    const [FormAlterarProduto, setFormAlterarProduto] = useState(false);

    function manipularSubmissao(evento){
        const form = evento.currentTarget;
        if (form.checkValidity()){
            //cadastrar o produto
           for(let i=0;i<props.listaDeProdutos.size();i++)
           {
                if(props.listaDeProdutos[i].codigo === produto.codigo)
                {
                    props.listaDeProdutos[i]=produto;
                }
           }
